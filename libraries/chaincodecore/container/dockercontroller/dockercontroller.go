...
import (
	...
	"github.com/fsouza/go-dockerclient"
	 ...
	 container "gocoin/libraries/chaincodecore/container/api"
	"gocoin/libraries/chaincodecore/container/ccintf"
	cutil "gocoin/libraries/chaincodecore/container/util"
	"github.com/spf13/viper"
	"golang.org/x/net/context"
)

var (
	dockerLogger = flogging.New("dockercontroller")
...
func getDockerHostConfig() *docker.HostConfig {
	...
	getInt64 := func(key string) int64 {
		defer func() {
			if err := recover(); err != nil {
				dockerLogger.Warn("load vm.docker.hostConfig.%s failed, error: %v", key, err)
			}
		}()
	...
	if err != nil {
		dockerLogger.Warn("load docker HostConfig.LogConfig failed, error: %s", err.Error())
	}
	networkMode := viper.GetString(dockerKey("NetworkMode"))
	if networkMode == "" {
		networkMode = "host"
	}
	dockerLogger.Debug("docker container hostconfig NetworkMode: %s", networkMode)
...
func (vm *DockerVM) deployImage(client dockerClient, ccid ccintf.CCID,
	...
	if err := client.BuildImage(opts); err != nil {
		dockerLogger.Error("Error building images: %s", err)
		dockerLogger.Error("Image Output:\n********************\n%s\n********************", outputbuf.String())
		return err
	}
...
func (vm *DockerVM) Start(ctxt context.Context, ccid ccintf.CCID,
	...
	if err != nil {
		...
		if err == docker.ErrNoSuchImage {
			if builder != nil {
				dockerLogger.Debug("start-could not find image <%s> (container id <%s>), because of <%s>..."+
					"attempt to recreate image", imageID, containerID, err)

				reader, err1 := builder()
				if err1 != nil {
					dockerLogger.Error("Error creating image builder for image <%s> (container id <%s>), "+
				...
				if err1 = vm.createContainer(ctxt, client, imageID, containerID, args, env, attachStdout); err1 != nil {
					dockerLogger.Error("start-could not recreate container post recreate image: %s", err1)
					return err1
				}
			} else {
				dockerLogger.Error("start-could not find image <%s>, because of %s", imageID, err)
				return err
			}
		} else {
			dockerLogger.Error("start-could not recreate container <%s>, because of %s", containerID, err)
			return err
		}
	}
	...
	if attachStdout {
		...
		go func() {
			...
			case <-time.After(10 * time.Second):
				dockerLogger.Error("Timeout while attaching to IO channel in container %s", containerID)
				return
			}
			...
			containerLogger := flogging.New(containerID)
			...
			for {
				...
				if err2 != nil {
					switch err2 {
					case io.EOF:
						dockerLogger.Info("Container %s has closed its IO channel", containerID)
					default:
						dockerLogger.Error("Error reading container output: %s", err2)
					}
	...
	if err != nil {
		dockerLogger.Error("start-could not start container: %s", err)
		return err
	}
...
func (vm *DockerVM) Stop(ctxt context.Context, ccid ccintf.CCID, timeout uint, dontkill bool, dontremove bool) error {
	...
	client, err := vm.getClientFnc()
	if err != nil {
		dockerLogger.Debug("stop - cannot create client %s", err)
		return err
	}
...
func (vm *DockerVM) Destroy(ctxt context.Context, ccid ccintf.CCID, force bool, noprune bool) error {
	...
	client, err := vm.getClientFnc()
	if err != nil {
		dockerLogger.Error("destroy-cannot create client %s", err)
		return err
	}
	id = strings.Replace(id, ":", "_", -1)
	...
	if err != nil {
		dockerLogger.Error("error while destroying image: %s", err)
	} else {
...
func (vm *DockerVM) GetVMName(ccid ccintf.CCID) (string, error) {
	...
	if !r.MatchString(name) {
		dockerLogger.Error("Error constructing Docker VM Name. '%s' breaks Docker's repository naming rules", name)
