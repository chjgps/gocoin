...
import (
	...
	"gocoin/libraries/chaincodecore/chaincode/platforms/util"
	...
	for i, file := range files {
		go func(i int, file SourceDescriptor) {
			...
			if excluded == false {
				...
				logger.Debug("vendoring %s -> %s", origName, file.Name)
			}
...
func (goPlatform *Platform) GetDeploymentPayload(spec *pb.ChaincodeSpec) ([]byte, error) {
	...
	imports = filter(imports, func(pkg string) bool {
		...
		if _, ok := provided[pkg]; ok == true {
			logger.Debug("Discarding provided package %s", pkg)
			return false
		}
		...
		for goroot := range goroots {
			...
			if err == nil && exists {
				logger.Debug("Discarding GOROOT package %s", pkg)
				return false
			}
		}

		// Else, we keep it
		logger.Debug("Accepting import: %s", pkg)
		return true
	})
	...
	for dep := range deps {
		...
		logger.Debug("processing dep: %s", dep)
		...
		for gopath := range gopaths {
			...
			logger.Debug("checking: %s exists: %v", fqp, exists)
